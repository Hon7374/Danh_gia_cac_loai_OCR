...
def make_dashboard_image(summary: dict, rows: list[dict]) -> None:
    """Create the dashboard referenced by both the Markdown and DOCX reports."""
    fig = plt.figure(figsize=(14, 8), dpi=160, facecolor="#F5F8FC")
    grid = fig.add_gridspec(2, 2, height_ratios=[0.72, 2.4], hspace=0.28, wspace=0.24)

    cards = fig.add_subplot(grid[0, :])
    cards.axis("off")
    cards.set_title("OCR benchmark dashboard", fontsize=18, weight="bold", loc="left", pad=8, color="#0B1F33")
    card_values = [
        ("Total runs", summary.get("total_runs", len(rows)), "#0057B8"),
        ("Successful", summary.get("ok_runs", sum(row.get("status") == "ok" for row in rows)), "#00875A"),
        ("Errors", summary.get("error_runs", sum(row.get("status") == "error" for row in rows)), "#C1121F"),
        ("Skipped", summary.get("skipped_runs", sum(row.get("status") == "skipped" for row in rows)), "#E67E22"),
    ]
    for index, (label, value, color) in enumerate(card_values):
        x = index * 0.25 + 0.01
        cards.add_patch(
            plt.Rectangle(
                (x, 0.04),
        ...
        cards.text(x + 0.018, 0.46, str(value), transform=cards.transAxes, fontsize=20, weight="bold", color=color, va="center")
        cards.text(x + 0.018, 0.20, label, transform=cards.transAxes, fontsize=10, color="#52667A", va="center")

    labels = [row_label(row).replace("\n", " / ") for row in rows]
    quality = [float(row.get("quality_score") or 0) for row in rows]
    runtime = [float(row.get("elapsed_sec") or 0) for row in rows]
    palette = ["#0057B8" if row.get("variant") == "raw" else "#00A676" for row in rows]

    quality_ax = fig.add_subplot(grid[1, 0])
    runtime_ax = fig.add_subplot(grid[1, 1])
    for ax, values, title, xlabel in (
        (quality_ax, quality, "Quality score", "Score (0-100)"),
        (runtime_ax, runtime, "Runtime", "Seconds"),
    ):
    ...
    if any(value > 0 for value in runtime):
        runtime_ax.set_xscale("log")

    fig.savefig(ASSETS / "dashboard_benchmark.png", bbox_inches="tight")
    plt.close(fig)
...
def write_report(report: dict, summary: dict, rows: list[dict]) -> None:
    ...
    runtime_note = (
        "Báo cáo này dùng môi trường Python/Windows local của project; Docker không nằm trong phạm vi kiểm thử."
    )
...
> Trạng thái triển khai: bản demo hiện tại chưa có module xác minh chữ ký số/PAdES. Các sơ đồ và mục chữ ký số bên dưới là kiến trúc đề xuất, không phải kết quả chạy thực nghiệm.
...
2. Lưu phiên bản Python, package và model trong job để tái lập được môi trường local.
...
def main() -> None:
    ...
    make_dashboard_image(summary, rows)
